...
function set_upload_param(up, files, ret)
{
    if (ret == false)
    {
        ret = get_signature()
    ...
    g_object_name = key;
    for(var i in files){
        if (i!=null) { suffix = get_suffix(i);
            calculate_object_name(i);
        }
    }
...
var uploader = new plupload.Uploader({
	...
	init: {
		...
		FilesAdded: function(up, files) {
            ...
            set_upload_param(up, files, true);
		},
		...
		FileUploaded: function(up, file, info) {
            if (info.status == 200)
            {
                ...
                $.ajax({
                    ...
                    success: function (data) { // 返回的RequestResult的json对象
                        alert(data.url);
                        $("#picDivId").append("<img src=\""+data.url+"\"/>");
                    }
                });
            }
