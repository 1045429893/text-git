    ...
    init: {
        ...
        FileUploaded: function(up, file, info) {
            if (info.status == 200)
            {
                ...
                var ossUrl  = $("#getBucketInfoUrlId").val();
                var  OssUrlName=get_uploaded_object_name(file.name)
                $.ajax({
                    type: 'PUT',
                    url: ossUrl,
                    dataType: 'json',
                    ...
                    success: function (data) { // 返回的RequestResult的json对象
                        var Publicurl=data.host+"/"+OssUrlName;
                        $("#picDivId").append("<img src=\""+Publicurl+"\"  style=\"width: 80px;height: 80px\" />");
                        parent.document.getElementById('imgUrl').value=Publicurl;
