...
public class RedisHashDao extends RedisDatabase
{
    ...
    public RedisHashDao(String id, String passWord)
    {
        super(id, passWord);
    }
    ...
    /**
     * 删除哈希表 key 中的一个或多个指定域，不存在的域(field)将被忽略。
     * 被成功移除的域的数量，不包括被忽略的域。
     * @param key
     * @param fields
     */
     ...
     * 因为不存在的 key 被当作一个空哈希表来处理，所以对一个不存在
     * 的 key进行 HMGET操作将返回一个只带有 nil 值的表。
     * @param key
     * @param fields
     * @return
     */
